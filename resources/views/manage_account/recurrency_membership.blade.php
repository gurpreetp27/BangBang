...
<section id="plan_info" class="wi-full py-5 text-center all-margin">
        <div class="container">
            <div class="title-main wi-full">
                <h2>{{__('member.ACTIVATERECURRENCY')}}</h2>
...
@endif
    ...
    <section id="account_info" class="wi-full py-5 text-center all-margin" style="display: {{$type == 'current'? 'block':'none'}}">
        <div class="container">
        <form id="sign-up-form" class="sign-up-form" method="POST" action="{{ url('/manageaccount/recurrency/membership')}}">
        @csrf
          <input type="hidden" name="plan_id" class="plan_id" value="{{isset($plan)?$plan->id:'0'}}">
            <div class="row">
                <div class="col-md-8">
                        ...
                        <div class="account-section text-left">
                            <h5>{{ __('member.your_mem') }}</h5>
                                <div class="select-plan">
                                    <h5 class="plan_name">{{isset($plan)?$plan->name:''}}</h5>
                                    <span class="price pay_info_plan"><small>€</small>
                                    @if(isset($plan))
                                     {{floor($plan->amount)}} <sup>
                                    <?php $sub_val = explode('.',$plan->amount);?>
                                    .{{$sub_val[1]}}</sup></span>
                                    @endif
                                </div>
